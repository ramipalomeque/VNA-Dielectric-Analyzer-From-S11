...
def vna_proc_file(file_path, file_name):
    ...
    # Conversión de unidad de frecuencia según el header del archivo Touchstone.
    # Necesario porque archivos de CST usan GHz mientras que los del VNA usan Hz.
    # Sin esta conversión, las frecuencias del CST quedan 1e9 veces más chicas
    # y las funciones de alineación (Resample_S11, SUBsample_S11) no encuentran
    # correspondencia entre ambos conjuntos de datos.
    freq_scale = {'HZ': 1, 'KHZ': 1e3, 'MHZ': 1e6, 'GHZ': 1e9}.get(headers[0], 1)

    for line in file_list[header_index + 1:]:
        values = line.strip().split()
        # frecuencies = np.append(frecuencies, np.float32(values[0]))  # no normaliza unidades de frecuencia
        frecuencies = np.append(frecuencies, np.float32(values[0]) * freq_scale)
